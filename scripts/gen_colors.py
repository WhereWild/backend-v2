...
For each variable the script:
  1. Selects the smallest expert palette that has enough slots for all groups:
       ...
       >12 groups → Krzywinski 12-color, extended with hue-rotated repeats
       (no longer independently CVD-verified past 12, but distinct rather
       than colliding — needed for legends like ecoregions/biome that have
       more than 12 real categories)
...
def _build_candidates(palette: list[str], n: int) -> list[str]:
    """Return n distinct candidate colors derived from palette.

    n <= len(palette): farthest-point sampling picks the n most perceptually
    distinct real (verified CVD-safe) palette entries.

    n > len(palette): every palette entry is used once, then additional
    candidates are generated by hue-rotating the palette in further passes
    (15° per pass) so groups beyond the verified 12-color cap still get a
    distinct, if no longer independently CVD-verified, color rather than
    colliding with an already-assigned group.
    """
    if n <= len(palette):
        return [palette[i] for i in _farthest_point_slots(palette, n)]
    ...
    while len(candidates) < n:
        cycle += 1
        shift = 15.0 * cycle
        for base_hex in palette:
            if len(candidates) >= n:
                break
            L, C, H = hex_to_lch(base_hex)  # noqa: N806
            candidates.append(lch_to_hex(L, C, (H + shift) % 360.0))
    return candidates
...
def _assign_palette_slots(
    ...
    maximum spread before minimisation. When n_groups exceeds every available
    palette's size (12), _build_candidates extends the palette with
    hue-rotated repeats rather than crashing.
    """
    n_groups = len(groups)
    candidates = _build_candidates(palette, n_groups)
...
# ---------------------------------------------------------------------------
# Ordinal legends — stepped continuous-colormap colors
#
# Nominal legends (unordered categories) go through the qualitative
# Okabe-Ito/Tol/Krzywinski machinery above. Ordinal legends (salinity, snow
# regime, ...) instead get N evenly-spaced samples off each of the SAME
# continuous colormaps util/tiles.py renders ratio/interval layers with
# (viridis/plasma/inferno/magma/cividis/gray), one swatch per class in id
# order — the class's position in the selected colormap IS its color, so
# there's no separate "colorblind-safe variant" to compute (viridis-family
# colormaps are themselves already CVD-considered), and no hand-picked hex
# to maintain. Uses matplotlib directly (already a backend dependency —
# util/tiles.py's _get_cmap_lut does the same) rather than reimplementing
# colormap sampling in Lab/LCH space, so generation-time colors are
# byte-identical to what the tile renderer produces at request time.
# ---------------------------------------------------------------------------

# Mirrors util/tiles.py's SUPPORTED_COLORMAPS — kept as a separate constant
# since this script has no import-time dependency on the backend package.
_ORDINAL_COLORMAPS: list[str] = ['viridis', 'plasma', 'inferno', 'magma', 'cividis', 'gray']

def _stepped_colors(n: int, colormap_name: str) -> list[str]:
    from matplotlib import colormaps
    cmap = colormaps[colormap_name]
    if n <= 1:
        return [_rgb_to_hex(*cmap(0.5)[:3])]
    return [_rgb_to_hex(*cmap(i / (n - 1))[:3]) for i in range(n)]

def generate_ordinal_variable(classes: list[dict]) -> dict[str, dict]:
    """Return {colormap_name: {class_id: hex}} for an ordinal legend.

    Classes are stepped in id order (ordinal ids are assumed to already
    encode the natural low→high order — true for every ordinal legend in
    this catalog).
    """
    ordered = sorted(classes, key=lambda c: c['id'])
    return {
        name: dict(zip((c['id'] for c in ordered), _stepped_colors(len(ordered), name)))
        for name in _ORDINAL_COLORMAPS
    }

def _load_catalog_value_types(backend_dir: str) -> dict[str, str]:
    """Return {layer_id: value_type} read from config/gis/catalog.json."""
    path = os.path.join(backend_dir, 'config', 'gis', 'catalog.json')
    with open(path) as f:
        catalog = json.load(f)
    return {
        layer['id']: str(layer.get('value_type') or '').lower()
...
// Nominal (unordered) legends use 'colorblind' / 'achromatopsia' variants of
// their hand-picked qualitative palette. Ordinal (ordered) legends have no
// separate accessibility variant — their entry is instead keyed by every
// ColormapId, one stepped color set per continuous colormap, since the
// selected colormap IS the coloring mechanism for ordinal variables.
export type CbMode = 'colorblind' | 'achromatopsia' | ColormapId;
...
def _format_variable(layer_id: str, modes: dict[str, dict]) -> str:
    lines = [f'  {layer_id}: {{']
    for ts_key, entries in modes.items():
...
def main() -> None:
    ...
    value_types = _load_catalog_value_types(backend_dir)
    ...
    for filename in sorted(os.listdir(legends_dir)):
        ...
        if value_types.get(layer_id) == 'ordinal':
            print(f'  {layer_id}: {len(classes)} classes, ordinal → stepped colormaps ({", ".join(_ORDINAL_COLORMAPS)})')
            variables[layer_id] = {'modes': generate_ordinal_variable(classes), 'shapes': None}
            continue
        ...
        generated = generate_variable(classes)
        variables[layer_id] = {
            'modes': {'colorblind': generated['cb'], 'achromatopsia': generated['ach']},
            'shapes': generated['shapes'],
        }

    ts = _TS_HEADER
    ts += '\n'.join(_format_variable(lid, v['modes']) for lid, v in variables.items())
    ts += '\n};\n\n'
    ...
    ts += '\n'.join(
        _format_shapes_variable(lid, v['shapes']) for lid, v in variables.items() if v['shapes'] is not None
    )
    ts += '\n};\n\n'
    ...
    # Also write a backend-readable JSON so tile renderer can apply CB/ordinal
    # colormap overrides — util/tiles.py's _cb_colormap_for_layer looks up
    # {layer_id: {mode: {class_id: hex}}} regardless of whether "mode" is an
    # accessibility mode (colorblind/achromatopsia) or a colormap name.
    cb_json_path = os.path.join(backend_dir, 'config', 'gis', 'cb_colors.json')
    cb_json: dict = {
        lid: {mode: {str(k): v for k, v in entries.items()} for mode, entries in v['modes'].items()}
        for lid, v in variables.items()
    }
