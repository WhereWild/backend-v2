...
def test_main_runs_compute_for_all(capsys, monkeypatch):
    with patch("scripts.process_tree.get_taxon_by_id", return_value=_FAKE_TAXON), \
        ...
        pt.main()
    expected_layer_meta = {lay["id"]: lay for lay in _FAKE_LAYERS}
    mock_stats.assert_called_once_with(_FAKE_TAXON, layers=_FAKE_LAYERS, layer_meta=expected_layer_meta)
