...
LIVE_WEATHER_VARIABLES: dict[str, dict] = {
    ...
    "soil_moisture_0_to_7cm": {
        ...
        "hi": 0.5,
        "stops": _SOIL_MOIST,
        "fetch_as": "soil_moisture_0_to_10cm",
    },
    "soil_temperature_0_to_7cm": {
        ...
        "hi": 40.0,
        "stops": _BLUE_RED,
        "fetch_as": "soil_temperature_0_to_10cm",
    },
...
def _save_to_disk(model: str, ref_time: str, var_id: str, arr: np.ndarray, forecast_hours: int = 0) -> None:
    _DISK_CACHE_DIR.mkdir(parents=True, exist_ok=True)
    np.save(_disk_path(model, ref_time, var_id, forecast_hours), arr)


def _load_from_disk(model: str, ref_time: str, var_id: str, forecast_hours: int = 0) -> np.ndarray | None:
...
def _populate_cache_from_hits(model: str, ref_time: str, disk_hits: dict[str, np.ndarray], forecast_hours: int) -> None:
...
def _load_model(model: str, forecast_hours: int = 0) -> None:
    ...
    valid = (
        valid_times[1] if target_dt is None else min(valid_times, key=lambda t: abs((t - target_dt).total_seconds()))
    )
...
def render_weather_tile_bytes(
    variable_id: str, z: int, x: int, y: int, tile_size: int = 256, forecast_hours: int = 0
) -> bytes | None:
...
def render_aggregate_tile_bytes(
    variable_id: str, window: str, z: int, x: int, y: int, tile_size: int = 256, forecast_hours: int = 0
) -> bytes | None:
    """Render a tile from a pre-computed aggregate raster (.npy on disk)."""
    suffix = f"__f{forecast_hours:03d}h" if forecast_hours else ""
    npy_path = (
        Path(__file__).parent.parent / "data" / "gis" / "temporal" / "rasters" / f"{variable_id}_{window}{suffix}.npy"
    )
...
def temporal_raster_path(variable_id: str, window_hours: int, forecast_hours: int = 0) -> Path | None:
    """Return the prebuilt temporal raster path used for model feature sampling."""
    window_label = _TEMPORAL_WINDOW_LABELS.get(window_hours)
    if window_label is None:
        return None
    if forecast_hours == 0:
        return _TEMPORAL_RASTER_DIR / f"{variable_id}_{window_label}.npy"
    return _TEMPORAL_RASTER_DIR / f"{variable_id}_{window_label}__f{forecast_hours:03d}h.npy"
...
def sample_grid_for_tile(
    ...
    npy_path = temporal_raster_path(variable_id, window_hours, forecast_hours)
    if npy_path is None:
        return nan_tile
